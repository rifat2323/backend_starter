 ...
 * @param {JwtObj} data - The data to be encoded in the token
 * @param {string} types - The type of token to be created. Supported types are "access_token", "refresh_token", "security_token"
 * @returns {string} The created token
 * @throws {Error} If the token type is not supported or if a required environment variable is not set
 */
